...
def main():
 ...
 for model in j['models']:
  ...
  for brush in g['brushes']:
   ...
   if a.recombine_max_faces is not None:
    from cw_brush_reconstruction import reconstruct_parts
    parts,cert['reconstruction']=reconstruct_parts(parts,cert,a.recombine_max_faces)
    cert['partitioned']=len(parts)>1;cert['pieces']=len(parts)
 ...
 if a.recombine_max_faces is not None:
  report['reconstruction']=dict(enabled=True,max_faces=a.recombine_max_faces,additional_simplification_tolerance=0.,cross_source_brush_merges=False,
   original_output_brushes=sum(c['reconstruction']['original_pieces']*c['instances'] for c in certs),output_brushes=len(rows),
   recombined_unique_brushes=sum(c['reconstruction']['merged'] for c in certs),
   recombined_placed_brushes=sum(c['instances'] for c in certs if c['reconstruction']['merged']),
   max_relative_volume_error=max(max(c['reconstruction'].get('relative_volume_error',0.),c['reconstruction'].get('max_relative_volume_error',0.)) for c in certs),
   policy='Reuse supplied exterior planes only for volume-checked convex unions within a certified source brush partition; preserve unsupported partitions unchanged.')
