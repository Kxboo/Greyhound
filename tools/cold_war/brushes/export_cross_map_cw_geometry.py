...
def payload_bytes(root, model, evidence=None):
    # Native Greyhound captures may pack logical files into shared blobs.
    from audit_cw_clip_payloads import read_logical
    if evidence is None:
        evidence_path = root / 'evidence.json'
        evidence = json.loads(evidence_path.read_text()) if evidence_path.exists() else {}
    data = read_logical(root, evidence, model['payload']['file'])
    expected = model['payload'].get('allocated_bytes')
    if expected is not None and len(data) != expected:
        raise ValueError('Collision payload length differs from capture metadata')
    return data
...
def write_float_mesh_obj(root, models, destination):
    ...
    with path.open('w', encoding='utf-8', newline='\n') as stream:
        ...
        for index, model in enumerate(models):
            raw = payload_bytes(root, model, evidence)
            try:
