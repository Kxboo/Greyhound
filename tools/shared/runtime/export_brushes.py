...
def main():
    ...
    p.add_argument('--game',choices=('cw','bo4'),default='cw')
    p.add_argument('--capture',required=True,type=Path)
    p.add_argument('--output',required=True,type=Path)
    p.add_argument('--map-path',default='')
    # Greyhound resolves the stem itself so the export folder and the .map files
    # share one name; see CWRadiantExport::MapStem. Standalone runs fall back to
    # map_name() below.
    p.add_argument('--map-name',default='')
    p.add_argument('--auto-types',action='store_true')
    p.add_argument('--trigger-capture',type=Path)
    p.add_argument('--collmaps-root',type=Path,help='Root containing game/map/model.map local collision exports')
    p.add_argument('--float-triangles',action='store_true',
    ...
    try:
        ...
        name=a.map_name or map_name(key,resolved)
        ...
        if (a.capture/'model_collision_owners.json').exists():
            ...
            report['model_collmaps']=export_collmaps(normalized,a.capture,geometry,assignments,
                collmaps_root/'black_ops_cw'/name,reference)
        # The float-triangle branch is a second collision representation, not a
        # brush one. It ships as OBJ beside the prefabs so it can never be
        # mistaken for Radiant geometry or compiled as such.
        if a.float_triangles:
            status('Writing captured float collision triangles')
            from export_cross_map_cw_geometry import write_float_mesh_obj
            surfaces=staged/'collision_surfaces';surfaces.mkdir(parents=True,exist_ok=True)
            models=json.loads((a.capture/'clip_map_models.json').read_text())['models']
            document=write_float_mesh_obj(a.capture,models,surfaces)
            # Relative to its sidecar; publishing moves this directory.
            document['obj']='float-collision-triangles.obj'
            (surfaces/'float-collision-index.json').write_text(json.dumps(document,indent=2))
            report['float_collision_triangles']={k:v for k,v in document.items() if k!='models'}
            report['float_collision_triangles'].update(folder='metadata/collision_surfaces',
                obj='metadata/collision_surfaces/float-collision-triangles.obj',
